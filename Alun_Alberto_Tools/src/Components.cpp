...
void Mesh::debugRender() {
    ...
    if (ImGui::TreeNode("Renderer")) {
        ...
        if (ImGui::TreeNode("Material" )) {
			...
			lm::vec3 amb = mat.ambient;
			float amb_array[3] = { amb.x, amb.y, amb.z };
			ImGui::AddSpace(0, 5);
			if (ImGui::DragFloat3("Ambient", amb_array)) {
				mat.ambient = lm::vec3(amb_array[0], amb_array[1], amb_array[2]);
			}

			lm::vec3 dif = mat.diffuse;
			float dif_array[3] = { dif.x, dif.y, dif.z };
			ImGui::AddSpace(0, 5);
			if (ImGui::DragFloat3("Diffuse", dif_array)) {
				mat.diffuse = lm::vec3(dif_array[0], dif_array[1], dif_array[2]);
			}

			lm::vec3 spec = mat.specular;
			float spec_array[3] = { spec.x, spec.y, spec.z };
			ImGui::AddSpace(0, 5);
			if (ImGui::DragFloat3("Specular", spec_array)) {
				mat.specular = lm::vec3(spec_array[0], spec_array[1], spec_array[2]);
			}
...
void Entity::Save(rapidjson::Document& json, rapidjson::Value & entity) {
    ...
    auto& faller = ECS.getSafeComponentFromEntity<Faller>(name);
    ...
    if (faller.index != 0) faller.Save(json, entity);
