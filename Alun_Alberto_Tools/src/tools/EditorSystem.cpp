		...
		ImGui::Begin("Add new component", &is_editor_mode);
		{
			...
			if (entity_id != -1) {
				...
				if (ImGui::Button("Add selected component")) {
					...
					is_adding_component = false;
					AddComponentSelected(selected_component);
				}
			}
		}
		ImGui::End();
    }
	if (is_deleting_component) {
		int entity_id = ECS.getEntity(selected);
		bool has_camera = false;
		for (auto& cam : ECS.getAllComponents<Camera>()) {
			if (cam.owner == entity_id) {
				has_camera = true;
				break;
			}
		}
		if (has_camera) {
			console_module_->ConsoleWrite(false, "If you try to delete a camera it would crash!!\n");
			is_deleting_component = false;
		} else {
...
void EditorSystem::AddComponentSelected(int id) {
	...
	case 1:
		...
		break;
	case 2:
		ECS.createComponentForEntity<Faller>(entity_id);
		break;
	default:
		break;
	}
